 ...
 * @min_size_member: The last memember of the type @kdst points in the initial
 *                   version
 * Return 0 for success, otherwise -error.
 */
#define iommu_copy_struct_from_user(kdst, user_data, min_size_member) \
	__iommu_copy_struct_from_user(kdst, user_data, sizeof(*kdst), \
				      offsetofend(typeof(*kdst), min_size_member))

/**
 * __iommu_copy_struct_from_user_array - Copy iommu driver specific user space data
 *                                     from an iommu_user_data_array input
 * @dst_data: Pointer to an iommu driver specific user data that is defined in
 *            include/uapi/linux/iommufd.h
 * @src_array: Pointer to a struct iommu_user_data_array for user space data array
...
__iommu_copy_struct_from_user_array(void *dst_data,
				     ...
				     unsigned int index, size_t data_len,
				     size_t min_len)
{
	...
	return __iommu_copy_struct_from_user(dst_data, &src_data,
					     data_len, min_len);
}

/**
 * iommu_copy_struct_from_user_array - Copy iommu driver specific user space data
 *                                   from an iommu_user_data_array input
 * @kdst: Pointer to an iommu driver specific user data that is defined in
 *        include/uapi/linux/iommufd.h
 * @user_array: Pointer to a struct iommu_user_data_array for user space data array
 * @min_size_member: The last memember of the type @kdst points in the initial
 *                   version
 * Return 0 for success, otherwise -error.
 */
#define iommu_copy_struct_from_user_array(kdst, user_array,                  \
					  index, min_size_member)             \
	__iommu_copy_struct_from_user_array(kdst, user_array, index,          \
					    sizeof(*kdst),                    \
