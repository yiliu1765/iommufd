					     ...
					     enum iommufd_object_type type)
{
	 ...
	 * the caller hasn't initalized it yet.
	 */
 ...
 * system call that created the object is guarenteed to succeed.
 */
			       ...
			       unsigned long arg)
{
	...
	if (ucmd.new_object) {
		if (ret)
			iommufd_object_abort(ucmd.ictx, ucmd.new_object);
		else
			iommufd_object_finalize(ucmd.ictx, ucmd.new_object);
	}
	return ret;
}
