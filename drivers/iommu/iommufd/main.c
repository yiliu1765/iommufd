...
static int iommufd_set_dev_data(struct iommufd_ucmd *ucmd)
{
	...
	union iommu_set_dev_data_user_data *data;
	struct iommufd_device *idev;
	const struct iommu_ops *ops;
	int rc;
	...
	if (!ops->set_dev_user_data || !ops->unset_dev_user_data ||
	    !ops->dev_user_data_len.min || !ops->dev_user_data_len.max) {
		rc = -EOPNOTSUPP;
		goto out_unlock;
	}

	if (cmd->data_len < ops->dev_user_data_len.min) {
		rc = -EINVAL;
		goto out_unlock;
	}

	/*
	 * Driver is buggy by providing an invalid range, or the dev_user data
	 * struct is missing in union iommu_set_dev_data_user_data.
	 */
	if (ops->dev_user_data_len.min > ops->dev_user_data_len.max ||
	    ops->dev_user_data_len.max > sizeof(*data)) {
		WARN_ON_ONCE(1);
		rc = -EINVAL;
		goto out_unlock;
	}
	...
	if (copy_struct_from_user(data, ops->dev_user_data_len.max,
...
#endif
	/*
	 * The union holding all hwpt_type specific structures used in the cache
	 * invalidation path
	 */
	union iommu_cache_invalidate_user_data cache_data;
};
