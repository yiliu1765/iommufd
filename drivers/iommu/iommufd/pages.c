 ...
 * iopt_pages represents a logical linear array of ful PFNs. The array is 0
 ...
 *  1) iopt_pages::pinned_pnfs xarray
 *  2) An iommu_domain
 *  3) The origin of the PFNs, the userspace pointer
 *
 ...
 * due to insufficent memory. In practice this means some cases have to hold
 ...
 *
 * In this file the term 'last' indicates an inclusive, closed, interval, eg
 * [0,0] refers to a single PFN. 'end' means an open range, eg [0,0) refers
 * to no PFNs.
 */
				   ...
				   unsigned long last_index)
{
	...
	if (last_index == iopt_area_last_index(area))
		...
		last_iova = iopt_area_index_to_iova(area, last_index + 1) - 1;
...
static int batch_to_domain(struct pfn_batch *batch, struct iommu_domain *domain,
			   unsigned long iova, unsigned long last_iova,
			   int iommu_prot)
{
...
static void iopt_pages_unpin(struct iopt_pages *pages,
			     struct pfn_batch *batch, unsigned long index,
			     unsigned long last)
{
				      ...
				      unsigned long last_index, bool force_fast)
{
	...
	if (force_fast ||
	    interval_tree_fully_covers_area(&area->pages->domains_itree, area) ||
	    interval_tree_fully_covers_area(&area->pages->users_itree, area)) {
	...
	while (cur_index != last_index + 1) {
		...
		batch_from_domain(&batch, domain,
				       iopt_area_index_to_iova(area, cur_index),
				 ...
				 struct iommu_domain *domain)
{
	...
	while (!pfn_reader_done(&pfns)) {
		rc = batch_to_domain(
			&pfns.batch, domain,
			iopt_area_index_to_iova(area, pfns.cur_index),
...
void iopt_area_unfill_domains(struct iopt_area *area)
{
	...
	iopt_area_unfill_whole_domain(area, area->storage_domain);
			      ...
			      unsigned long last)
{
	...
	if (interval_tree_fully_covers(&pages->domains_itree, index, last))
...
out_clean_xa:
	if (index != xa_end)
		iopt_pages_unfill_xarray(pages, index, xa_end - 1);
	pfn_reader_destroy(&pfns);
	return rc;
}
 ...
 *
 * This is used when adding a new domain to the iopt. Every area in the iopt
...
void iopt_unfill_domain(struct io_pagetable *iopt, struct iommu_domain *domain)
{
	...
	if (iopt->next_domain_id != 0) {
		/* Pick an arbitary remaining domain to act as storage */
