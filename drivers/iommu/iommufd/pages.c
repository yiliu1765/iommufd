			      ...
			      unsigned int flags)
{
	...
	while (cur < batch->end) {
		...
		copy_data_page(pfn_to_page(batch->pfns[cur] + npage),
			       data, offset, bytes, flags);
		offset = 0;
		length -= bytes;
		data += bytes;
		copied += bytes;
			     ...
			     unsigned long index, unsigned long last)
{
	...
	interval_tree_for_each_span(&user_span, &pages->access_itree, index,
				    last) {
			   ...
			   unsigned long index, unsigned long last)
{
	...
	pfns->upages_start = 0;
	pfns->upages_end = 0;
	interval_tree_span_iter_first(&pfns->span, &pages->access_itree, index,
				      last);
				      ...
				      unsigned long last_index)
{
	...
	if (interval_tree_fully_covers_area(&pages->domains_itree, area) ||
	    interval_tree_fully_covers_area(&pages->access_itree, area)) {
...
/*
 * Erase entries in the pinned_pfns xarray that are not covered by any accesses.
				    ...
				    unsigned long index, unsigned long last)
{
	...
	interval_tree_for_each_span (&span, &pages->access_itree, index, last)
		if (span.is_hole)
			      ...
			      unsigned long last)
{
	...
	/* FIXME: need to check the access_itree */
	...
	interval_tree_for_each_span (&span, &pages->access_itree, start, last) {
 ...
 *
 * This can be called if the caller is holding a refcount on an iopt_pages_access
			      ...
			      unsigned long length, unsigned int flags)
{
	...
	rc = pin_user_pages_remote(
		pages->source_mm,
		(uintptr_t)(pages->uptr + index * PAGE_SIZE), 1,
		flags & IOMMUFD_ACCESS_RW_WRITE ? FOLL_WRITE : 0, &page, NULL,
		NULL);
	...
	if (!(flags & IOMMUFD_ACCESS_RW_KTHREAD) && change_mm) {
		if (start_index == last_index)
			return iopt_pages_rw_page(pages, start_index,
						  ...
						  length, flags);
		mutex_lock(&pages->mutex);
		rc = iopt_pages_rw_slow(pages, start_index, last_index,
			  ...
			  unsigned int flags)
{
	...
	access = iopt_pages_get_exact_access(pages, start, last);
	if (access) {
