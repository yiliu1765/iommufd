...
__mock_domain_alloc_paging(unsigned int iommu_domain_type, bool nest_parent)
{
			   ...
			   const struct iommu_hwpt_selftest *user_cfg)
{
	...
	if (!mock_parent || !mock_parent->nest_parent)
		return ERR_PTR(-EINVAL);
...
static struct iommu_domain *mock_domain_alloc(unsigned int iommu_domain_type)
{
	...
	if (iommu_domain_type != IOMMU_DOMAIN_UNMANAGED)
		return NULL;
	domain = __mock_domain_alloc_paging(iommu_domain_type, 0);
	if (IS_ERR(domain))
		domain = NULL;
	return domain;
}
...
{
	bool nest_parent = flags & IOMMU_HWPT_ALLOC_NEST_PARENT;
...
static int mock_domain_set_dev_data_user(struct device *dev,
					 const struct iommu_user_data *user_data)
{
...
static void mock_domain_unset_dev_data_user(struct device *dev)
{
	struct mock_dev *mdev = container_of(dev, struct mock_dev, dev);
...
static const struct iommu_ops mock_ops = {
	...
	.set_dev_data_user = mock_domain_set_dev_data_user,
	.unset_dev_data_user = mock_domain_unset_dev_data_user,
	.default_domain_ops =
