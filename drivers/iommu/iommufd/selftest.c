						   ...
						   const union iommu_domain_user_data *user_data)
{
	...
	const struct iommu_hwpt_selftest *user_cfg =
		(const struct iommu_hwpt_selftest *)user_data;
			...
			const union iommu_set_dev_data_user_data *user_data)
{
	...
	const struct iommu_test_device_data *data =
		(const struct iommu_test_device_data *)user_data;
...
static const struct iommu_ops mock_ops = {
	...
	.domain_alloc_user_data_len =
		IOMMU_USER_DATA_LENGTH(struct iommu_hwpt_selftest, test_config),
	.capable = mock_domain_capable,
	.set_platform_dma_ops = mock_domain_set_plaform_dma_ops,
	.set_dev_user_data = mock_domain_set_dev_user_data,
	.unset_dev_user_data = mock_domain_unset_dev_user_data,
	.dev_user_data_len =
		IOMMU_USER_DATA_LENGTH(struct iommu_test_device_data, val),
	.default_domain_ops =
...
{
	const struct iommu_hwpt_invalidate_selftest *inv_info =
		(const struct iommu_hwpt_invalidate_selftest *)user_data;
	struct mock_iommu_domain *mock =
		container_of(domain, struct mock_iommu_domain, domain);
...
static struct iommu_domain_ops domain_nested_ops = {
	...
	.cache_invalidate_user_data_len = {
		.min = offsetofend(struct iommu_hwpt_invalidate_selftest, flags),
		.max = sizeof(struct iommu_hwpt_invalidate_selftest),
	},
};
