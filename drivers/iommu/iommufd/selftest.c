					 ...
					 struct iommu_domain *old)
{
	...
	/*
	 * First attach with pasid 1024 succeeds, second attach would fail.
	 * This is helpful to test the case in which the iommu core needs
	 * to rollback to old domain due to driver failure.
	 * mdev->pasid_1024_fake_error is used to mark if the fake error of
	 * the replace is ready to go.
	 */
	if (pasid == 1024) {
		if (domain->type == IOMMU_DOMAIN_BLOCKED) {
			...
			/*
			 * Fake an error to fail the replacement and
			 * clear the flag.
			 */
			atomic_set(&mdev->pasid_1024_fake_error, 0);
			return -ENOMEM;
...
static struct mock_dev *mock_dev_create(unsigned long dev_flags)
{
	...
	fwspec->iommu_fwnode = mock_iommu.iommu_dev.fwnode;
...
int __init iommufd_test_init(void)
{
	...
	mock_iommu.iommu_dev.fwnode = kzalloc(sizeof(struct iommu_fwspec),
					      GFP_KERNEL);
	if (!mock_iommu.iommu_dev.fwnode) {
		rc = -ENOMEM;
		goto err_bus;
	}
...
err_fw:
	kfree(mock_iommu.iommu_dev.fwnode);
	mock_iommu.iommu_dev.fwnode = NULL;
err_bus:
