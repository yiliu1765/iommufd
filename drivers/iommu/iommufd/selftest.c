		       ...
		       const struct iommu_user_data *user_data)
{
	...
	switch (data_type) {
	case IOMMU_HWPT_ALLOC_DATA_NONE:
		if (user_data || parent)
			return ERR_PTR(-EINVAL);
		alloc_fn = __mock_domain_alloc_kernel;
		break;
	default:
		/* IOMMU_HWPT_ALLOC_DATA_SELFTEST cannot be a case for a big value */
		if (data_type != IOMMU_HWPT_ALLOC_DATA_SELFTEST)
			return ERR_PTR(-EINVAL);
		if (!user_data || !parent ||
	...
	if (user_data) {
		int rc = iommu_copy_struct_from_user(&data, user_data, iotlb);
		if (rc)
			return ERR_PTR(rc);
		user_cfg = &data;
	}
					 ...
					 const struct iommu_user_data *user_data)
{
	...
	rc = iommu_copy_struct_from_user(&data, user_data, val);
	if (rc)
		return rc;
