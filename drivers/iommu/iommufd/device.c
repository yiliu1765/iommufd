...
void iommufd_device_destroy(struct iommufd_object *obj)
{
	...
	if (idev->has_user_data)
		dev_iommu_ops(idev->dev)->unset_dev_data_user(idev->dev);
...
static int iommufd_hw_pagetable_enforce_rr(struct iommufd_hw_pagetable *hwpt,
					   struct iommufd_device *idev)
{
	...
	/* Do not overwrite igroup->sw_msi_start if msi_cookie is setup */
	if (hwpt_paging->msi_cookie)
		sw_msi_start = NULL;
...
/* Must be called when new_ioas == old_ioas */
				    ...
				    struct iommufd_hw_pagetable *new_hwpt)
{
	 ...
	 */
	if (old_hwpt->obj.type == IOMMUFD_OBJ_HWPT_NESTED &&
	    new_hwpt->obj.type == IOMMUFD_OBJ_HWPT_NESTED)
		return 0;

	/*
	 * Even if the new hw_pagetable is a NESTED hwpt managed by user space,
	 * it has to enforce IOMMU_RESV_SW_MSI (only!) on its associated PAGING
	 * hw_pagetable managed by the kernel.
	 */
	if (new_hwpt->obj.type == IOMMUFD_OBJ_HWPT_NESTED)
		sw_msi_only = true;
	else
		sw_msi_only = false;

	old_ioas = to_hwpt_paging(old_hwpt)->ioas;
	new_ioas = to_hwpt_paging(new_hwpt)->ioas;
				...
				struct iommufd_device *idev)
{
	...
	rc = iommufd_hw_pagetable_enforce_rr(hwpt, idev);
	if (rc)
		goto err_unlock;
	...
	return 0;
err_unresv:
	iommufd_hw_pagetable_remove_rr(hwpt, idev);
err_unlock:
	mutex_unlock(&idev->igroup->lock);
	return rc;
}
			  ...
			  struct iommufd_hw_pagetable *hwpt)
{
	...
	if (to_hwpt_paging(hwpt)->ioas != to_hwpt_paging(old_hwpt)->ioas) {
		list_for_each_entry(cur, &igroup->device_list, group_item) {
			rc = iommufd_hw_pagetable_enforce_rr(hwpt, cur);
			if (rc)
				goto err_unresv;
		}
	...
	if (to_hwpt_paging(hwpt)->ioas != to_hwpt_paging(old_hwpt)->ioas) {
		list_for_each_entry(cur, &igroup->device_list, group_item)
			iommufd_hw_pagetable_remove_rr(old_hwpt, cur);
	}
	igroup->hwpt = hwpt;
...
err_unresv:
	if (to_hwpt_paging(hwpt)->ioas != to_hwpt_paging(old_hwpt)->ioas) {
		list_for_each_entry(cur, &igroup->device_list, group_item)
			iommufd_hw_pagetable_remove_rr(hwpt, cur);
	} else {
			       ...
			       attach_fn do_attach)
{
	...
	hwpt = iommufd_hw_pagetable_alloc_paging(idev->ictx, ioas, idev,
						 0, immediate_attach);
	if (IS_ERR(hwpt)) {
...
{
	struct iommu_set_idev_data *cmd = ucmd->cmd;
	...
	if (cmd->data_type == IOMMU_IDEV_DATA_NONE)
		return -EINVAL;
	...
	rc = ops->set_dev_data_user(idev->dev, &user_data);
	if (rc)
		goto out_unlock;
...
{
	struct iommu_unset_idev_data *cmd = ucmd->cmd;
	struct iommufd_device *idev;
	int rc = 0;
