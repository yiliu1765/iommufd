				  ...
				  unsigned long length)
{
	...
	pages = area->pages;
	*start_byte = iopt_area_start_byte(area, iova);
	kref_get(&pages->kref);
	up_read(&iopt->iova_rwsem);
...
EXPORT_SYMBOL_NS_GPL(iopt_access_pages, IOMMUFD);
...
EXPORT_SYMBOL_NS_GPL(iopt_unaccess_pages, IOMMUFD);

/**
 * iopt_access_rw - Read or write data under the iova
 * @iopt: io_pagetable to act on
 * @iova: Starting IOVA
		   ...
		   unsigned long length, unsigned int flags)
{
	...
	for (area = iopt_area_iter_first(iopt, iova, last_iova); area;
		...
		bytes = (last - cur_iova) + 1;
		rc = iopt_pages_rw_access(area->pages,
					  iopt_area_start_byte(area, cur_iova),
					  data, bytes, flags);
		if (rc)
			break;
...
EXPORT_SYMBOL_NS_GPL(iopt_access_rw, IOMMUFD);
