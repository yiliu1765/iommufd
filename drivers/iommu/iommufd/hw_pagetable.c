				...
				bool dummy)
{
	...
	/*
	 * Driver is buggy by missing cache_invalidate_user or having an invalid
	 * cache_invalidate_user_data_len range.
	 */
	if (!hwpt->domain->ops->cache_invalidate_user ||
	...
	/*
	 * The invalidate data struct used by the driver is missing in union
	 * iommu_cache_invalidate_user_data
	 */
	if (hwpt->domain->ops->cache_invalidate_user_data_len.max >
	    sizeof(union iommu_cache_invalidate_user_data)) {
		WARN_ON_ONCE(1);
		rc = -EINVAL;
		goto out_abort;
	}
			   ...
			   bool immediate_attach)
{
	...
	/* Driver is buggy by mixing user-managed stuff in kernel-managed ops */
	if (hwpt->domain->ops->cache_invalidate_user ||
	    hwpt->domain->ops->cache_invalidate_user_data_len.min ||
	    hwpt->domain->ops->cache_invalidate_user_data_len.max) {
		WARN_ON_ONCE(1);
