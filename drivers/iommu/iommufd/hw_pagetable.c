...
void iommufd_hw_pagetable_destroy(struct iommufd_object *obj)
{
	...
	if (hwpt->type == IOMMUFD_HWPT_KERNEL) {
		...
		WARN_ON(!list_empty(&hwpt->kernel.child_domains));
		...
		list_del(&hwpt->user.child_domains_item);
			      ...
			      struct iommufd_ioas *ioas, struct device *dev)
{
	...
	kernel = &hwpt->kernel;
	kernel->ioas = ioas;
	INIT_LIST_HEAD(&kernel->child_domains);
			     ...
			     struct device *dev)
{
	...
	case IOMMUFD_OBJ_HW_PAGETABLE: {
		...
		if (!hwpt->domain) {
			...
			if (hwpt->type == IOMMUFD_HWPT_USER) {
...
int iommufd_hwpt_alloc(struct iommufd_ucmd *ucmd)
{
	...
	INIT_LIST_HEAD(&hwpt->devices);
	mutex_init(&hwpt->devices_lock);
	hwpt->type = IOMMUFD_HWPT_USER;
	user = &hwpt->user;
	user->parent = parent;
	user->pgtbl_ptr = cmd->pgtbl_ptr;
