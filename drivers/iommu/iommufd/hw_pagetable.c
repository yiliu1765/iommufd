 ...
 * @data_type: Requested type of hw_pagetable
				...
				bool dummy)
{
	...
	hwpt->domain = ops->domain_alloc_user(idev->dev, flags, data_type,
			   ...
			   bool immediate_attach)
{
	...
	if (user_data && data_type == IOMMU_HWPT_ALLOC_DATA_NONE)
		return ERR_PTR(-EINVAL);
...
int iommufd_hwpt_alloc(struct iommufd_ucmd *ucmd)
{
	struct iommufd_hw_pagetable *(*alloc_fn)(
					...
					struct iommufd_object *pt_obj,
					struct iommufd_device *idev,
					u32 flags, enum iommu_hwpt_data_type type,
					struct iommu_user_data *user_data,
					bool flag);
	...
	if ((cmd->flags & (~IOMMU_HWPT_ALLOC_NEST_PARENT)) || cmd->__reserved)
		return -EOPNOTSUPP;
	if (!cmd->data_len && cmd->data_type != IOMMU_HWPT_ALLOC_DATA_NONE)
		return -EINVAL;
