					...
					unsigned long type)
{
	 ...
	 * Creating an iopt_pages_access that spans the whole area will
	 * immediately stop new faults as they will be handled from the xarray.
	 * We can then reparent the iopt_pages to the new mm_struct and undo the
	 * iopt_pages_access. No blockage of kernel  required, does require
