				    ...
				    struct iommu_attach_handle *handle)
{
	...
	if (handle) {
		...
		entry = xa_tag_pointer(handle, IOMMU_PASID_ARRAY_HANDLE);
	} else {
		entry = xa_tag_pointer(domain, IOMMU_PASID_ARRAY_DOMAIN);
	}
...
static void *iommu_tag_pasid_array_entry(struct iommu_domain *domain,
					 struct iommu_attach_handle *handle)
{
	if (handle)
			       ...
			       struct iommu_attach_handle *handle)
{
	...
	mutex_lock(&group->mutex);
	entry = iommu_tag_pasid_array_entry(domain, handle);
