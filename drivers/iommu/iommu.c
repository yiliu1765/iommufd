 ...
 * then the group's domain will be set back to the existing domain if
 * must_succeed is false, otherwise an error is returned and the group's domains
 * is left inconsistent. This is a driver bug to fail attach with a previously
 * good domain. We try to avoid a kernel UAF because of this.
 *
