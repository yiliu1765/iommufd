					   ...
					   struct dmar_domain *dmar_domain)
{
	 ...
	 */
	if (dmar_domain->domain.type == IOMMU_DOMAIN_SVA) {
...
static void intel_iommu_remove_dev_pasid(struct device *dev, ioasid_t pasid)
{
	...
	if (WARN_ON_ONCE(!domain))
		return;
	device_pasid_block_translation(dev, pasid, to_dmar_domain(domain));
}
			      ...
			      struct device *dev, ioasid_t pasid)
{
	struct device_domain_info *info = dev_iommu_priv_get(dev);
	struct dmar_domain *dmar_domain = to_dmar_domain(domain);
	struct iommu_domain *audit_domain = domain;
	struct intel_iommu *iommu = info->iommu;
	struct dev_pasid_info *dev_pasid;
	...
	old_domain = iommu_get_domain_for_dev_pasid(dev, pasid, 0);
	if (old_domain)
		device_pasid_block_translation(dev, pasid,
					       to_dmar_domain(old_domain));

	/*
	 * Nested type domain should prepare its parent domain for
	 * attachment instead of itself.
	 */
	if (domain_type_is_nested(dmar_domain))
		audit_domain = &dmar_domain->s2_domain->domain;

	ret = prepare_domain_attach_device(audit_domain, dev);
	if (ret)
		return ret;
