...
/*
 * This function commits modifications to an existing pasid table entry.
 * This includes flushing cache line and caches to make the latest pasid
 * table entry take effect.
 */
static void pasid_entry_commit_update(struct intel_iommu *iommu,
				      struct device *dev,
				 ...
				 u32 pasid, bool fault_ignore)
{
	...
	pasid_entry_commit_update(iommu, dev, pasid, did, pgtt, pte);
}
...
static void
intel_pasid_make_first_stage_only_pte(struct pasid_entry *pte,
...
{
	struct pasid_entry *pte, old_pte = {}, tmp_pte = {};
	...
	intel_pasid_make_first_stage_only_pte(&tmp_pte, iommu, pgd, did, flags);

	pasid_set_entry(pte, &tmp_pte);

	spin_unlock(&iommu->lock);

	if (pasid_entry_empty(&old_pte))
		pasid_entry_commit_update(iommu, dev, pasid,
					  pasid_get_domain_id(&old_pte),
					  pasid_pte_get_pgtt(&old_pte),
					  pte);
	else
...
static void
intel_pasid_make_second_stage_only_pte(struct pasid_entry *pte,
				       struct intel_iommu *iommu,
				       u64 pgd_val, u16 did,
				       int agaw, bool dirty_tracking)
{
	pasid_set_domain_id(pte, did);
				   ...
				   struct device *dev, u32 pasid)
{
	...
	intel_pasid_make_second_stage_only_pte(&tmp_pte, iommu, pgd_val, did,
					       agaw, domain->dirty_tracking);
...
static void 
intel_pasid_make_passthrough_pte(struct pasid_entry *pte,
				 struct intel_iommu *iommu,
				 u16 did)
{
				   ...
				   struct device *dev, u32 pasid)
{
	...
	intel_pasid_make_passthrough_pte(&tmp_pte, iommu, did);
...
static void 
intel_pasid_make_nested_pte(struct pasid_entry *pte,
			    struct intel_iommu *iommu,
			     ...
			     u32 pasid, struct dmar_domain *domain)
{
	...
	intel_pasid_make_nested_pte(&tmp_pte, iommu, s1_cfg, domain);
