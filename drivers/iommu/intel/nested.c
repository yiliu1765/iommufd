					      ...
					      void *user_data)
{
	...
	for (index = 0; index < entry_nr; index++) {
		...
		if (req->__reserved) {
			pr_err_ratelimited("Invalid invalidation data, non-zero reserved field\n");
			ret = -EINVAL;
			break;
		}

		if (req->flags & (~IOMMU_VTD_QI_FLAGS_LEAF)) {
			pr_err_ratelimited("Invalid invalidation data, invalid flags\n");
			ret = -EINVAL;
			break;
		}

		if (!IS_ALIGNED(req->addr, VTD_PAGE_SIZE)) {
			pr_err_ratelimited("Invalid invalidation address 0x%llx\n",
					    req->addr);
			ret = -EINVAL;
			break;
		}
	...
	if (index < entry_nr &&
	    put_user(index, (uint32_t __user *)u64_to_user_ptr(nr_uptr)))
		return -EFAULT;
