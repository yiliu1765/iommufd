					  ...
					  struct device *dev, ioasid_t pasid)
{
	...
	domain_detach_iommu(ndomain->s2_domain, iommu);
}
...
static void intel_nested_invalidate(struct dmar_domain *domain,
				    struct device *dev,
...
{
	struct nested_domain *ndomain = to_nested_domain(domain);
	struct dmar_domain *s2_domain = ndomain->s2_domain;
	struct device_domain_info *info;

	mutex_lock(&ndomain->mutex);
	list_for_each_entry(info, &ndomain->devices, nested)
		intel_nested_invalidate(s2_domain, info->dev,
					user_data, data_len);
	mutex_unlock(&ndomain->mutex);
}
					       ...
					       void *user_data, size_t data_len)
{
	...
	ndomain->s1_cfg = *vtd;
	ndomain->domain.type = IOMMU_DOMAIN_NESTING;
