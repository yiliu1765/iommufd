				      ...
				      const struct iommu_user_data *user_data)
{
	...
	ret = iommu_copy_struct_from_user(&user, user_data, sid);
	if (ret)
		return ret;
			   ...
			   const struct iommu_user_data *user_data)
{
	...
	if (data_type != IOMMU_HWPT_ALLOC_DATA_NONE &&
	    data_type != IOMMU_HWPT_ALLOc_DATA_ARM_SMMUV3)
		return ERR_PTR(-EINVAL);
	if (data_type == IOMMU_HWPT_ALLOC_DATA_NONE && user_data)
		return ERR_PTR(-EINVAL);
