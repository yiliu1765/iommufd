					  ...
					  const struct iommu_user_data *user_data)
{
	const size_t min_len =
		offsetofend(struct iommu_hwpt_arm_smmuv3_invalidate, __reserved);
	...
	ret = iommu_copy_user_data(&inv, user_data, sizeof(inv), min_len);
	if (ret)
		return ret;

	q.llq.prod = inv.cmdq_prod;
	q.llq.max_n_shift = inv.cmdq_log2size;
	q.base = (__le64 *)u64_to_user_ptr(inv.cmdq_uptr);
	q.ent_dwords = inv.cmdq_entry_size / sizeof(u64);
