...
static int arm_smmu_set_dev_user_data(struct device *dev,
				const union iommu_set_dev_user_data *user_data)
{
	const struct iommu_dev_data_arm_smmuv3 *user =
		(const struct iommu_dev_data_arm_smmuv3 *)user_data;
	struct arm_smmu_master *master = dev_iommu_priv_get(dev);
			...
			const union iommu_cache_invalidate_user_data *user_data)
{
	...
	struct arm_smmu_queue q = {
		...
		.base = (__le64 *)u64_to_user_ptr(inv->cmdq_uptr),
	...
	ncmds = min(queue_ncmds(&q.llq), CMDQ_BATCH_ENTRIES - 1);

	cmds = kcalloc(ncmds, inv->cmdq_entry_size, GFP_KERNEL);
	if (!cmds)
		return -ENOMEM;

	for (; !queue_empty(&q.llq); queue_inc_cons(&q.llq)) {
		u64 *cmd = &cmds[i * CMDQ_ENT_DWORDS];

		ret = copy_from_user(cmd,
				     (const void __user *)Q_ENT(&q, q.llq.cons),
				     inv->cmdq_entry_size);
		if (ret) {
			ret = -EFAULT;
			goto out_free_cmds;
		...
		if (arm_smmu_cmdq_hit_errata(smmu, cmd, &has_leaf, &has_cfgi)) {
...
static const struct iommu_domain_ops arm_smmu_nested_domain_ops = {
	...
	.cache_invalidate_user_data_len =
		IOMMU_USER_DATA_LENGTH(struct iommu_hwpt_arm_smmuv3_invalidate,
				       __reserved),
};
...
{
	const struct iommu_hwpt_arm_smmuv3 *user_cfg = (struct iommu_hwpt_arm_smmuv3 *)user_data;
	struct arm_smmu_master *master = dev_iommu_priv_get(dev);
...
static struct iommu_ops arm_smmu_ops = {
	...
	.domain_alloc_user_data_len =
		IOMMU_USER_DATA_LENGTH(struct iommu_hwpt_arm_smmuv3,
				       out_event_uptr),
	...
	.dev_user_data_len	=
		IOMMU_USER_DATA_LENGTH(struct iommu_dev_data_arm_smmuv3, sid),
