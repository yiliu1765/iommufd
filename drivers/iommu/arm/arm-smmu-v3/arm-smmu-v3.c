...
static int arm_smmu_set_dev_data_user(struct device *dev,
				      const struct iommu_user_data *user_data)
{
	struct arm_smmu_master *master = dev_iommu_priv_get(dev);
	struct arm_smmu_stream *stream = &master->streams[0];
	struct arm_smmu_device *smmu = master->smmu;
	struct iommu_idev_data_arm_smmuv3 user;
	u32 sid_user;
	int ret = 0;

	ret = iommu_copy_struct_from_user(&user, user_data,
					  IOMMU_IDEV_DATA_ARM_SMMUV3, sid);
	if (ret)
		return ret;
...
static void arm_smmu_unset_dev_data_user(struct device *dev)
{
	struct arm_smmu_master *master = dev_iommu_priv_get(dev);
	struct arm_smmu_stream *stream = &master->streams[0];
	struct arm_smmu_device *smmu = master->smmu;
...
__arm_smmu_domain_alloc(unsigned type,
			...
			struct arm_smmu_master *master, u32 flags,
			const struct iommu_hwpt_arm_smmuv3 *user_cfg)
{
	bool flag_s2 = flags & IOMMU_HWPT_ALLOC_NEST_PARENT;
	struct arm_smmu_domain *smmu_domain;
	struct iommu_domain *domain;
	int ret = 0;
	...
	if (master) {
		...
		ret = arm_smmu_domain_finalise(domain, master, flag_s2, user_cfg);
		if (ret) {
			kfree(smmu_domain);
			return NULL;
		}
...
{
	return __arm_smmu_domain_alloc(type, NULL, NULL, 0, NULL);
}
			   ...
			   const struct iommu_user_data *user_data)
{
	...
	if (user_data) {
		int ret = iommu_copy_struct_from_user(&data, user_data,
	...
	return __arm_smmu_domain_alloc(type, s2, master, flags, user_cfg);
}
...
static struct iommu_ops arm_smmu_ops = {
	...
	.set_dev_data_user	= arm_smmu_set_dev_data_user,
	.unset_dev_data_user	= arm_smmu_unset_dev_data_user,
