...
static struct vfio_device *vfio_device_get_from_cdev(struct file *filep)
{
	/* device file may be not opened from vfio device cdev, so
	 * i_cdev possible to be NULL. */
	if (!filep->f_inode->i_cdev)
		return ERR_PTR(-EOPNOTSUPP);

	return container_of(filep->f_inode->i_cdev, struct vfio_device, cdev);
}
...
static long vfio_device_bind_iommufd(struct file *filep, unsigned long arg)
{
	...
	device = vfio_device_get_from_cdev(filep);
	if (IS_ERR(device))
	...
	ret = device->ops->bind_iommufd(device, iommufd, &bind.out_devid);
	if (ret)
		goto out_unlock;
				       ...
				       unsigned int cmd, unsigned long arg)
{
	 ...
	 * if the device fd is opened via cdev. */
