...
bool vfio_iommufd_enforced_coherent(struct vfio_device *vdev)
{
	bool ret = true;

	mutex_lock(&vdev->dev_set->lock);
	if (vdev->iommufd_device)
		ret = iommufd_device_enforced_coherent(vdev->iommufd_device);
...
int vfio_iommufd_physical_attach_ioas(struct vfio_device *vdev, u32 *pt_id)
{
	...
	ret = iommufd_device_attach(vdev->iommufd_device, pt_id, flags);
	if (ret)
		return ret;
