				       ...
				       size_t argsz)
{
	...
	pci_read_config_dword(pdev, pdev->pasid_cap + PCI_PASID_CAP, &cap_ctrl);
#endif

	if (cap_ctrl & PCI_PASID_CAP_EXEC)
		pasid.capabilities |= VFIO_DEVICE_PASID_CAP_EXEC;
	if (cap_ctrl & PCI_PASID_CAP_PRIV)
		pasid.capabilities |= VFIO_DEVICE_PASID_CAP_PRIV;

	pasid.width = (cap_ctrl >> 8) & 0x1f;
