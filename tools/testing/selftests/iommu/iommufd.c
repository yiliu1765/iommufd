...
TEST_F(iommufd_ioas, get_hw_info_exact_size)
{
	struct iommu_test_hw_info info;

	if (self->device_id) {
		test_cmd_get_hw_info(self->device_id, &info, sizeof(info));
	} else {
...
TEST_F(iommufd_ioas, get_hw_info_larger_size)
{
	void *info =
		malloc(sizeof(struct iommu_test_hw_info) + 64);

	ASSERT_NE(NULL, info);

	if (self->device_id) {
		test_cmd_get_hw_info(self->device_id, info, sizeof(*info) + 64);
	} else {
		test_err_get_hw_info(ENOENT, self->device_id,
				     info, sizeof(*info) + 64);
	}
