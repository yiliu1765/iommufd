...
TEST_F(iommufd_ioas, hwpt_alloc)
{
	...
	if (self->stdev_id && self->device_id) {
		test_cmd_hwpt_alloc(self->device_id, self->ioas_id, 0, &new_hwpt_id);
		...
		test_err_hwpt_alloc(ENOENT, self->device_id,
				    self->ioas_id, 0, &new_hwpt_id);
...
TEST_F(iommufd_ioas, nested_hwpt_alloc)
{
	...
	if (self->device_id) {
		...
		test_err_hwpt_alloc_user(EINVAL, self->device_id,
					 parent_hwpt_id, 0,
		...
		test_err_hwpt_alloc_user(EINVAL, self->device_id,
					 ...
					 min_data_len - 1, &data,
					 &nested_hwpt_id[0]);
		test_err_hwpt_alloc_user(EFAULT, self->device_id,
					 parent_hwpt_id, 0,
					 IOMMU_HWPT_TYPE_SELFTEST,
					 sizeof(data), NULL,
					 &nested_hwpt_id[0]);
		...
		test_cmd_hwpt_alloc_user(self->device_id, parent_hwpt_id, 0,
					 IOMMU_HWPT_TYPE_SELFTEST,
					 sizeof(data), &data,
					 &nested_hwpt_id[0]);
		test_cmd_hwpt_alloc_user(self->device_id, parent_hwpt_id, 0,
		...
		test_err_hwpt_alloc(ENOENT, self->device_id, self->ioas_id,
				    0, &parent_hwpt_id);
		test_err_hwpt_alloc_user(ENOENT, self->device_id,
					 parent_hwpt_id, 0,
					 IOMMU_HWPT_TYPE_SELFTEST,
					 sizeof(data), &data,
					 &nested_hwpt_id[0]);
...
TEST_F(iommufd_mock_domain, alloc_hwpt)
{
	...
	for (i = 0; i != variant->mock_domains; i++) {
		...
		test_err_hwpt_alloc(EOPNOTSUPP, self->idev_ids[0],
				    self->ioas_id,
				    ~IOMMU_HWPT_ALLOC_NEST_PARENT,
				    &hwpt_id);
		test_cmd_hwpt_alloc(self->idev_ids[0], self->ioas_id, 0,
				    &hwpt_id);
