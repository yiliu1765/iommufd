...
TEST_F(iommufd_ioas, hwpt_alloc)
{
	...
	if (self->stdev_id && self->device_id) {
		...
		test_err_cmd_hwpt_alloc_user(EINVAL,
					     ...
					     IOMMU_HWPT_TYPE_DEFAULT,
					     &data, min_data_len);
		test_cmd_hwpt_alloc_user(self->device_id, self->ioas_id, 0,
					 &new_hwpt_id[1],
					 IOMMU_HWPT_TYPE_DEFAULT, NULL, 0);
...
TEST_F(iommufd_ioas, nested_hwpt_alloc)
{
	...
	if (self->device_id) {
		...
		num_inv = 1;
		test_err_cmd_hwpt_invalidate(EINVAL, parent_hwpt_id, inv_reqs,
					     sizeof(*inv_reqs), &num_inv, NULL);
		/* Negative test: structure size sanity */
		num_inv = 1;
		test_err_cmd_hwpt_invalidate(EINVAL, nested_hwpt_id[0], inv_reqs,
