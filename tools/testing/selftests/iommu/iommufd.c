...
#define TEST_LENGTH(_struct, _ioctl, _last)                              \
	...
	TEST_LENGTH(iommu_hwpt_invalidate, IOMMU_HWPT_INVALIDATE, data_uptr);
	...
	TEST_LENGTH(iommu_set_dev_data, IOMMU_SET_DEV_DATA, data_len);
	TEST_LENGTH(iommu_unset_dev_data, IOMMU_UNSET_DEV_DATA, dev_id);
...
TEST_F(iommufd_ioas, nested_hwpt_alloc)
{
	const uint32_t min_data_len =
		offsetofend(struct iommu_hwpt_selftest, test_config);
	...
	if (self->device_id) {
		...
		test_err_cmd_hwpt_alloc_user(EINVAL, self->device_id,
					     ...
					     min_data_len - 1, &data);
		test_err_cmd_hwpt_alloc_user(EFAULT, self->device_id,
					     parent_hwpt_id,
					     &nested_hwpt_id[0],
					     IOMMU_HWPT_TYPE_SELFTEST,
					     sizeof(data), NULL);
...
TEST_F(iommufd_device_pasid, pasid_attach)
{
	if (self->device_id) {
		 ...
		 * iommufd does not support it.
		 */
		test_err_cmd_pasid_attach(ENOENT, 100, self->ioas_id);
		...
		/*
		 * Attach nested hwpt to pasid 100, should succeed, domain should
		 * be valid.
		 */
		...
		/*
		 * Replace pasid 200 without attaching it first,
		 * should fail with -EINVAL.
		 */
		test_err_cmd_pasid_replace(EINVAL, pasid, parent_hwpt_id);

		/*
		 * Attach s2 hwpt to pasid 200, should succeed, domain should
		 * be valid.
		 */
		...
		/*
		 * Replace pasid 200 with self->ioas_id, should fail since
		 * iommufd does not support it
		 */
		test_err_cmd_pasid_attach(ENOENT, pasid, self->ioas_id);

		/*
		 * Replace with another nested hwpt for pasid 200, should succeed,
